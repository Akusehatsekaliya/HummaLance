    <div class="freelance-projects bg-home bg-white py-5">
        <div class="container">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h3 style="font-size: 1.5em; font-weight: bold;">Most Populer Freelance Service</h3>
            </div>
            <br>
            <ul class="custom-navbar">
                <li class="custom-nav-item" style="color: #01ABFE">
                    <a class="custom-nav-linkPopular">Popular Service</a>
                </li>
            </ul>
            <br>
            <br>
            <div class="row ">
                <div class="col">
                    {{-- <div class="freelancer"> --}}
                    <div class="freelancer2">
                        <img src="images/senatopcustudio.jpg">
                        <div class="freelancer2-footer">
                            <img src="images/user/s7.png">
                            <h5>Clocker</h5>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;"></p>
                            <p class="badge1"
                                style="text-align: left; padding-left: 9px; font-weight: bold; margin-right: 20px; margin-top: -50px; padding: 2px 10px; font-size: 10px; font-weight: bold;">
                                Top Rated
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                            </p>
                        </div>

                        <p style="text-align: left; padding-left: 10px; font-weight: 500; margin-top: -12px;">I Will
                            Make You
                            A Great Logo Design, With Modern And Minimalist Style</p>
                        <div class="content-info" style="padding-left: 10px;">
                            <div class="rating-wrapper">
                                <span class="gig-rating text-body-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1792 1792" width="15"
                                        height="15">
                                        <path fill="currentColor"
                                            d="M1728 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5t-30.5 14.5q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 0-31.5-14.5t-10.5-35.5q0-6 2-20l86-500-364-354q-25-27-25-48 0-37 56-46l502-73 225-455q19-41 49-41t49 41l225 455 502 73q56 9 56 46z">
                                        </path>
                                    </svg>
                                    <span style="font-weight: bold">5.0 </span>(555)
                                </span>
                            </div>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;">From $20</p>
                            <button class="btn btn-success" type="submit"
                                style="margin-right: 18px; margin-top: -15px; padding: 6px 15px; font-size: 12px; font-weight: bold;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                    viewBox="0 0 24 24">
                                    <path fill="currentColor"
                                        d="M2 22V4q0-.825.588-1.412T4 2h16q.825 0 1.413.588T22 4v12q0 .825-.587 1.413T20 18H6zm4-8h8v-2H6zm0-3h12V9H6zm0-3h12V6H6z" />
                                </svg>
                                Order Now
                            </button>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div class="freelancer2">
                        <img src="images/senatopcustudio.jpg">
                        <div class="freelancer2-footer">
                            <img src="images/user/s7.png">
                            <h5>Clocker</h5>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;"></p>
                            <p class="badge1"
                                style="text-align: left; padding-left: 9px; font-weight: bold; margin-right: 20px; margin-top: -50px; padding: 2px 10px; font-size: 10px; font-weight: bold;">
                                Top Rated
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                            </p>
                        </div>

                        <p style="text-align: left; padding-left: 10px; font-weight: 500; margin-top: -12px;">I Will
                            Make You
                            A Great Logo Design, With Modern And Minimalist Style</p>
                        <div class="content-info" style="padding-left: 10px;">
                            <div class="rating-wrapper">
                                <span class="gig-rating text-body-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1792 1792" width="15"
                                        height="15">
                                        <path fill="currentColor"
                                            d="M1728 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5t-30.5 14.5q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 0-31.5-14.5t-10.5-35.5q0-6 2-20l86-500-364-354q-25-27-25-48 0-37 56-46l502-73 225-455q19-41 49-41t49 41l225 455 502 73q56 9 56 46z">
                                        </path>
                                    </svg>
                                    <span style="font-weight: bold">5.0 </span>(555)
                                </span>
                            </div>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;">From $20</p>
                            <button class="btn btn-success" type="submit"
                                style="margin-right: 18px; margin-top: -15px; padding: 6px 15px; font-size: 12px; font-weight: bold;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                    viewBox="0 0 24 24">
                                    <path fill="currentColor"
                                        d="M2 22V4q0-.825.588-1.412T4 2h16q.825 0 1.413.588T22 4v12q0 .825-.587 1.413T20 18H6zm4-8h8v-2H6zm0-3h12V9H6zm0-3h12V6H6z" />
                                </svg>
                                Order Now
                            </button>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div class="freelancer2">
                        <img src="images/senatopcustudio.jpg">
                        <div class="freelancer2-footer">
                            <img src="images/user/s7.png">
                            <h5>Clocker</h5>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;"></p>
                            <p class="badge1"
                                style="text-align: left; padding-left: 9px; font-weight: bold; margin-right: 20px; margin-top: -50px; padding: 2px 10px; font-size: 10px; font-weight: bold;">
                                Top Rated
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                            </p>
                        </div>

                        <p style="text-align: left; padding-left: 10px; font-weight: 500; margin-top: -12px;">I Will
                            Make You
                            A Great Logo Design, With Modern And Minimalist Style</p>
                        <div class="content-info" style="padding-left: 10px;">
                            <div class="rating-wrapper">
                                <span class="gig-rating text-body-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1792 1792" width="15"
                                        height="15">
                                        <path fill="currentColor"
                                            d="M1728 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5t-30.5 14.5q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 0-31.5-14.5t-10.5-35.5q0-6 2-20l86-500-364-354q-25-27-25-48 0-37 56-46l502-73 225-455q19-41 49-41t49 41l225 455 502 73q56 9 56 46z">
                                        </path>
                                    </svg>
                                    <span style="font-weight: bold">5.0 </span>(555)
                                </span>
                            </div>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;">From $20</p>
                            <button class="btn btn-success" type="submit"
                                style="margin-right: 18px; margin-top: -15px; padding: 6px 15px; font-size: 12px; font-weight: bold;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                    viewBox="0 0 24 24">
                                    <path fill="currentColor"
                                        d="M2 22V4q0-.825.588-1.412T4 2h16q.825 0 1.413.588T22 4v12q0 .825-.587 1.413T20 18H6zm4-8h8v-2H6zm0-3h12V9H6zm0-3h12V6H6z" />
                                </svg>
                                Order Now
                            </button>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div class="freelancer2">
                        <img src="images/senatopcustudio.jpg">
                        <div class="freelancer2-footer">
                            <img src="images/user/s7.png">
                            <h5>Clocker</h5>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;"></p>
                            <p class="badge1"
                                style="text-align: left; padding-left: 9px; font-weight: bold; margin-right: 20px; margin-top: -50px; padding: 2px 10px; font-size: 10px; font-weight: bold;">
                                Top Rated
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13"
                                    viewBox="0 0 48 48">
                                    <path fill="currentColor" fill-rule="evenodd"
                                        d="m16.871 33.044l-.873 11.66c-.062.82.667 1.383 1.318 1.018c4.613-2.584 19.062-12.074 26.386-32.932c.336-.957-.567-1.82-1.337-1.28c-4.326 3.03-13.78 9.29-19.623 10.484c0 0 3.742-2.6 5.981-6.588a1.2 1.2 0 0 0-.043-1.245L20.513 2.389c-.484-.697-1.478-.408-1.652.481l-2.543 12.937L4.384 26.223c-.598.522-.476 1.577.215 1.856zm21.104-4.596c-.756 1.055-2.384 3.224-3.887 4.55c-.3.265-.26.709.084.925l7.943 4.986c.478.3 1.123-.056 1.043-.576c-.37-2.383-1.434-7.351-4.125-9.959c-.3-.29-.819-.26-1.059.074"
                                        clip-rule="evenodd" />
                                </svg>
                            </p>
                        </div>

                        <p style="text-align: left; padding-left: 10px; font-weight: 500; margin-top: -12px;">I Will
                            Make You
                            A Great Logo Design, With Modern And Minimalist Style</p>
                        <div class="content-info" style="padding-left: 10px;">
                            <div class="rating-wrapper">
                                <span class="gig-rating text-body-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1792 1792" width="15"
                                        height="15">
                                        <path fill="currentColor"
                                            d="M1728 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5t-30.5 14.5q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 0-31.5-14.5t-10.5-35.5q0-6 2-20l86-500-364-354q-25-27-25-48 0-37 56-46l502-73 225-455q19-41 49-41t49 41l225 455 502 73q56 9 56 46z">
                                        </path>
                                    </svg>
                                    <span style="font-weight: bold">5.0 </span>(555)
                                </span>
                            </div>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <p style="text-align: left; padding-left: 10px; font-weight: bold;">From $20</p>
                            <button class="btn btn-success" type="submit"
                                style="margin-right: 18px; margin-top: -15px; padding: 6px 15px; font-size: 12px; font-weight: bold;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                    viewBox="0 0 24 24">
                                    <path fill="currentColor"
                                        d="M2 22V4q0-.825.588-1.412T4 2h16q.825 0 1.413.588T22 4v12q0 .825-.587 1.413T20 18H6zm4-8h8v-2H6zm0-3h12V9H6zm0-3h12V6H6z" />
                                </svg>
                                Order Now
                            </button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
